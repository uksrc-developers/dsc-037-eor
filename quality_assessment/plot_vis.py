...
# Plot helpers
# ----------------------
def plot_or_save_plots(results, outdir, show=False):
    """
    Save all plots to output directory or just display if show=True and do not block.

    Parameters
    ----------
    results : dict
        Results dictionary containing processed visibility/QA arrays and metadata.
    outdir : str
        Output directory to save plots if show is False.
    show : bool, optional
        If True, only plot/show figures (do not close); if False (default), save as PNG.
    """
    ...
    plt.tight_layout()
    f1 = os.path.join(outdir, f"vis_amp_time_{tag}.png") if not show else None
    figs.append(fig1)
    if not show:
    ...
    plt.tight_layout()
    f2 = os.path.join(outdir, f"vis_phs_time_{tag}.png") if not show else None
    figs.append(fig2)
    if not show:
    ...
    plt.tight_layout()
    f3 = os.path.join(outdir, f"vis_amp_freq_{tag}.png") if not show else None
    figs.append(fig3)
    if not show:
    ...
    plt.tight_layout()
    f4 = os.path.join(outdir, f"vis_phs_freq_{tag}.png") if not show else None
    figs.append(fig4)
    if not show:
    ...
    if show:
        ...
        print("[INFO] Displayed 4 plots. Close all windows to continue.")
        plt.show(block=True)
        # Return figures so they don't get garbage collected
        return tuple(figs)
    else:
        return f1, f2, f3, f4
...
Examples:
  ...
  python plot_vis.py input_file.ms --ant1 2 --ant2 6 --show
        """
    )
    ...
    # Call the main processing function, passing the show parameter
